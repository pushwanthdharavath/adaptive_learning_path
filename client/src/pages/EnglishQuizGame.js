...
const EnglishQuizGame = () => {
  console.log('🔍 Initializing EnglishQuizGame component...');
  ...
  const fetchInitialQuestions = async (emotion = null) => {
    ...
    console.log('🔍 Fetching English questions with:', { level: levelNum, emotion, correctAnswers, wrongAnswers });
    try {
      ...
      console.log('✅ English questions fetched:', response.data);
  ...
  const completeLevel = async (levelScore) => {
    console.log('🏁 Completing English level with score:', levelScore);
    ...
    try {
      ...
      try {
        ...
        console.log('✅ English game progress saved successfully');
      } catch (error) {
        console.error('Error saving English game progress:', error);
      }
  ...
  const startLevel = async () => {
    console.log('🔍 Starting English level...');
    try {
      setLoading(true);
      setError(null);
  ...
  const startNextLevel = () => {
    ...
    navigate(`/english-quiz/level/${nextLevel}`);
  };
  ...
  const handlePlayAgain = async () => {
    ...
    try {
      ...
      console.log('✅ English game progress reset successfully');
      navigate('/english-quiz-levels');
    } catch (error) {
    ...
    return (
      <div className="math-quiz-container">
        <div className="quiz-header">
          <h1 className="quiz-title">English Challenge</h1>
        </div>
