...
const ScienceQuizGame = () => {
  console.log('🔍 Initializing ScienceQuizGame component...');
  ...
  const fetchInitialQuestions = async (emotion = null) => {
    ...
    console.log('🔍 Fetching Science questions with:', { level: levelNum, emotion, correctAnswers, wrongAnswers });
    try {
      ...
      console.log('✅ Science questions fetched:', response.data);
  ...
  const completeLevel = async (levelScore) => {
    console.log('🏁 Completing Science level with score:', levelScore);
    ...
    try {
      ...
      try {
        ...
        console.log('✅ Science game progress saved successfully');
      } catch (error) {
        console.error('Error saving Science game progress:', error);
      }
  ...
  const startLevel = async () => {
    console.log('🔍 Starting Science level...');
    try {
      setLoading(true);
      setError(null);
  ...
  const startNextLevel = () => {
    ...
    navigate(`/science-quiz/level/${nextLevel}`);
  };
  ...
  const handlePlayAgain = async () => {
    ...
    try {
      ...
      console.log('✅ Science game progress reset successfully');
      navigate('/science-quiz-levels');
    } catch (error) {
    ...
    return (
      <div className="math-quiz-container">
        <div className="quiz-header">
          <h1 className="quiz-title">Science Challenge</h1>
        </div>
